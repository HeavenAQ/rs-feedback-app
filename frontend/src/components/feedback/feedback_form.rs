...
pub fn FeedbackForm() -> Html {
    ...
    let handle_select = {
        ...
        Callback::from(move |value| rating.set(value))
    };
    ...
    let on_submit = {
        ...
        Callback::from(move |event: SubmitEvent| {
            ...
            spawn_local(async move {
                ...
                let response = create_feedback(feedback_data.to_string().as_str()).await;
                match response {
                    Ok(feedback) => {
                        set_loading(false, dispatch.clone());
                        set_show_alert(
                            "Feedback added successfully!".to_string(),
                            dispatch.clone(),
                        );
                        set_feedback(feedback, dispatch.clone());
                    }
                    Err(err) => {
                        set_loading(false, dispatch.clone());
                        set_show_alert(err.to_string(), dispatch.clone());
                    }
                }
            });
        })
